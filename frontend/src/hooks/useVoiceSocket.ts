...
export function useVoiceSocket(roomId: string | null, options: VoiceSocketOptions = {}): VoiceSocketApi {
  ...
  const sendChat = useCallback((content: string, extra: Record<string, any> = {}) => {
    ...
    upsertChat?.(optimistic);
    return emit('chat:send', { roomId: activeRoomRef.current, content: content.trim(), ...extra, clientId: id }, undefined, 5);
  }, [emit, upsertChat]);
  ...
  const connect = useCallback(async () => {
    ...
    if (reconnectingRef.current) return socketRef.current;
    ...
    socket.on('chat:sent', msg => upsertChat?.({ ...msg, pending: false }));
    ...
    socket.on('server:error', payload => {
      updateStatus('error', payload?.message || payload?.error || 'Server error');
    });
