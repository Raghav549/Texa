...
const normalizeEnum = (value: unknown) => normalizeString(value).toUpperCase();
...
const roomForTrending = (region: string) => `trending:${normalizeString(region).toLowerCase() || 'global'}`;
...
const verifyReelAccess = async (reelId: string, userId: string): Promise<ReelAccessResult> => {
  ...
  const moderationStatus = normalizeEnum((reel as any).moderationStatus);
  ...
  const visibility = normalizeEnum((reel as any).visibility);
...
const getEngagementCounts = async (reelId: string) => {
  const [reel, comments, saves] = await Promise.all([
    ...
    (prisma as any).reelSave?.count
      ? (prisma as any).reelSave.count({ where: { reelId } }).catch(() => null)
...
const updateReelDerivedMetrics = async (tx: any, reelId: string) => {
  ...
  const saveCount = tx.reelSave?.count
    ? await tx.reelSave.count({ where: { reelId } }).catch(() => getSaveCount(reel))
    : getSaveCount(reel);
...
const flushWatchProgress = async (io: Server, key: string) => {
  ...
  const duration = clamp(Math.max(1, normalizeNumber(payload.duration, (access.reel as any).duration || DEFAULT_REEL_DURATION)), 1, MAX_POSITION_SECONDS);
  ...
  const recentView = await (prisma as any).viewHistory?.findFirst
    ? (prisma as any).viewHistory.findFirst({
        where: {
          reelId,
          userId,
          watchedAt: {
  ...
  const previousAnyView = await (prisma as any).viewHistory?.findFirst
    ? (prisma as any).viewHistory.findFirst({
        where: {
          reelId,
          userId
  ...
  await prisma.$transaction(async tx => {
    if ((tx as any).watchProgress?.upsert) {
      await (tx as any).watchProgress.upsert({
        where: {
          reelId_userId: {
            reelId,
            userId
    ...
    if (!recentView && progress >= 0.08 && (tx as any).viewHistory?.create) {
      await (tx as any).viewHistory.create({
        data: {
          reelId,
          userId,
      ...
      await tx.reel.update({
        ...
        data: {
          ...
          uniqueViews: previousAnyView ? undefined : {
            increment: 1
          },
          watchTime: {
            increment: position
    ...
    if (recentView && (tx as any).viewHistory?.update) {
      ...
      await (tx as any).viewHistory.update({
        where: {
    ...
    if ((tx as any).viewHistory?.aggregate) {
      const aggregate = await (tx as any).viewHistory.aggregate({
        where: {
          reelId
        },
        _count: true,
      ...
      const completedCount = await (tx as any).viewHistory.count({
        where: {
          reelId,
          completed: true
        }
...
const handleShare = async (reelId: string, userId: string, value: any) => {
  await prisma.$transaction(async tx => {
    if ((tx as any).reelShare?.create) {
      await (tx as any).reelShare.create({
        data: {
          reelId,
          userId,
...
const handleSave = async (reelId: string, userId: string, value: any) => {
  await prisma.$transaction(async tx => {
    if ((tx as any).reelSave?.upsert) {
      await (tx as any).reelSave.upsert({
        where: {
          reelId_userId: {
            reelId,
            userId
      ...
      const saves = Array.isArray((reel as any)?.saves) ? (reel as any).saves : [];
...
const handleUnsave = async (reelId: string, userId: string) => {
  await prisma.$transaction(async tx => {
    if ((tx as any).reelSave?.deleteMany) {
      await (tx as any).reelSave.deleteMany({
        where: {
          reelId,
          userId
        }
      ...
      await tx.reel.update({
        ...
        data: {
          saves: {
            set: saves.filter((id: string) => id !== userId)
          }
        } as any
      }).catch(() => null);
    }
...
export function initReelSockets(io: Server) {
  ...
  ns.on('connection', (socket: Socket) => {
    ...
    socket.on('reel:join', async (payload: any = {}, ack?: Ack) => {
      try {
        ...
        const [counts, progress] = await Promise.all([
          ...
          (prisma as any).watchProgress?.findUnique
            ? (prisma as any).watchProgress.findUnique({
                where: {
                  reelId_userId: {
                    reelId,
    ...
    socket.on('comment:typing:stop', async (payload: any = {}, ack?: Ack) => {
      const reelId = normalizeString(payload?.reelId);
    ...
    socket.on('trending:subscribe', (payload: any = {}, ack?: Ack) => {
      const region = normalizeString(payload?.region).toLowerCase() || 'global';
