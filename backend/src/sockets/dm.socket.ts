...
function isSafeEmoji(value: any) {
...
export function initDMSockets(io: Server) {
  ...
  ns.use(async (socket: Socket, next) => {
    try {
      ...
      if (!userId || !isValidId(userId)) return next(new Error('Invalid authentication token'));
  ...
  ns.on('connection', async (socket: Socket) => {
    ...
    const role = (socket.data as SocketUserData).role || null;
    ...
    socket.on('conversation:join', async ({ conversationId }: ConversationPayload = {}, ack?: Ack) => {
      try {
        ...
        socket.join(room(CONVERSATION_ROOM_PREFIX, conversationId));
        ...
        const unread = participant
          ? await prisma.message.count({
              where: {
                conversationId,
                senderId: { not: userId },
                createdAt: { gt: (participant as any).lastReadAt || new Date(0) },
                deletedAt: null,
    ...
    socket.on('message:send', async (payload: any, ack?: Ack) => {
      try {
        ...
        const allowed = await canAccessConversation(payload.conversationId, userId, role);
    ...
    socket.on('message:seen', async ({ messageIds, conversationId }: MessageSeenPayload = {}, ack?: Ack) => {
      try {
        ...
        uniqueSenders.forEach(senderId => {
          ns.to(room(USER_ROOM_PREFIX, senderId)).emit('message:seen', readPayload);
        });

        ns.to(room(CONVERSATION_ROOM_PREFIX, conversationId)).emit('conversation:seen', {
          conversationId,
          userId,
          lastReadMessageId: latest.id,
          timestamp: new Date().toISOString()
    ...
    socket.on('message:reaction', async ({ messageId, emoji, remove }: ReactionPayload = {}, ack?: Ack) => {
      try {
        ...
        if (!message || (message as any).deletedAt) return safeAck(ack, { success: false, error: 'Message not found' });

        const allowed = await canAccessConversation((message as any).conversationId, userId, role);
        if (!allowed) return safeAck(ack, { success: false, error: 'Not a participant' });
        ...
        ns.to(room(CONVERSATION_ROOM_PREFIX, (message as any).conversationId)).emit('message:reaction:broadcast', {
          messageId,
    ...
    socket.on('call:offer', async ({ conversationId, offer, targetUserId, type }: CallPayload = {}, ack?: Ack) => {
      try {
        ...
        ns.to(room(USER_ROOM_PREFIX, targetUserId)).emit('call:incoming', {
          from: userId,
          conversationId,
          offer,
    ...
    socket.on('call:answer', async ({ conversationId, answer, targetUserId }: CallPayload = {}, ack?: Ack) => {
      try {
        ...
        ns.to(room(USER_ROOM_PREFIX, targetUserId)).emit('call:accepted', {
          from: userId,
          conversationId,
          answer,
          timestamp: new Date().toISOString()
    ...
    socket.on('call:ice-candidate', async ({ conversationId, candidate, targetUserId }: CallPayload = {}, ack?: Ack) => {
      try {
        ...
        ns.to(room(USER_ROOM_PREFIX, targetUserId)).emit('call:ice-candidate', {
          from: userId,
          conversationId,
          candidate,
          timestamp: new Date().toISOString()
    ...
    socket.on('call:end', async ({ conversationId, targetUserId, reason }: CallPayload = {}, ack?: Ack) => {
      try {
        ...
        ns.to(room(USER_ROOM_PREFIX, targetUserId)).emit('call:ended', {
          from: userId,
          conversationId,
          reason: reason || 'ended',
          timestamp: new Date().toISOString()
    ...
    socket.on('payment:initiate', async ({ conversationId, amount, recipientId }: PaymentInitiatePayload = {}, ack?: Ack) => {
      try {
        ...
        if (recipientId === userId) return safeAck(ack, { success: false, error: 'Cannot send coins to yourself' });
        ...
        ns.to(room(USER_ROOM_PREFIX, recipientId)).emit('payment:incoming', {
          messageId: message.id,
          conversationId,
          amount: coinAmount,
          from: userId,
      ...
      try {
        ...
        await prisma.$transaction(async tx => {
          ...
          const sender = await tx.user.findUnique({
            where: { id: (latestMessage as any).senderId },
            select: { coins: true }
          });
        ...
        ns.to(room(CONVERSATION_ROOM_PREFIX, (message as any).conversationId)).emit('payment:completed', {
          messageId,
          conversationId: (message as any).conversationId,
          amount,
    ...
    socket.on('poll:vote', async ({ messageId, optionIndex }: PollVotePayload = {}, ack?: Ack) => {
      try {
        ...
        const message = await getMessageWithConversation(messageId);
        if (!message || !(message as any).poll) return safeAck(ack, { success: false, error: 'Poll not found' });

        const allowed = await canAccessConversation((message as any).conversationId, userId, role);
        if (!allowed) return safeAck(ack, { success: false, error: 'Not a participant' });

        const poll = sanitizePoll((message as any).poll);
        ...
        ns.to(room(CONVERSATION_ROOM_PREFIX, (message as any).conversationId)).emit('poll:updated', {
          messageId,
          conversationId: (message as any).conversationId,
          poll,
          votedBy: userId,
    ...
    socket.on('disappearing:set', async ({ messageId, seconds }: DisappearingPayload = {}, ack?: Ack) => {
      try {
        ...
        if (message.senderId !== userId && !isAdminRole(role) && (participant as any)?.role !== 'admin') {
    ...
    socket.on('folder:update', async ({ folderId, conversationIds }: FolderUpdatePayload = {}, ack?: Ack) => {
      try {
        ...
        socket.emit('folder:updated', {
          folderId,
          conversationIds: (updated as any).conversationIds || allowedIds
        });
