...
import { redis, initRedisAdapter, connectRedis, closeRedisConnections } from "./config/redis";
...
const parseOrigins = (): CorsOptions["origin"] => {
  ...
  if (!list.length) return isProduction ? false : true;
...
const normalizeRole = (role?: string | null) => String(role || "USER").toUpperCase();

const isAdminRole = (role?: string | null) => {
  const value = normalizeRole(role);
  return value === "ADMIN" || value === "SUPERADMIN" || value === "SUPER_ADMIN" || value === "OWNER";
};
...
const rateLimitKeyGenerator = (req: Request) => {
  return String(req.headers["x-device-id"] || req.headers["x-forwarded-for"] || req.ip || "unknown");
};
...
io.use(async (socket: Socket, next) => {
  try {
    if (state.shuttingDown) return next(new Error("Server is shutting down"));
    ...
    const token =
      typeof rawToken === "string"
        ...
        : Array.isArray(rawToken)
          ? String(rawToken[0] || "").replace(/^Bearer\s+/i, "").trim()
          : "";
    ...
    socket.data.role = normalizeRole(decoded.role || user.role || "USER");
...
io.on("connection", socket => {
  ...
  const role = normalizeRole(socket.data.role || "USER");
  ...
  socket.on("typing:start", ({ roomId, conversationId } = {}) => {
    ...
    if (conversationId) {
      socket.to(`dm:${conversationId}`).emit("typing:start", {
        userId,
        conversationId,
        timestamp: new Date().toISOString()
      });
  ...
  socket.on("typing:stop", ({ roomId, conversationId } = {}) => {
    ...
    if (conversationId) {
      socket.to(`dm:${conversationId}`).emit("typing:stop", {
        userId,
        conversationId,
        timestamp: new Date().toISOString()
      });
  ...
  socket.on("admin:join", () => {
    if (isAdminRole(role)) socket.join("admin");
  });
