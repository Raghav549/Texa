...
const boolish = z.union([z.boolean(), z.string().transform(value => ["true", "1", "yes", "on"].includes(value.toLowerCase()))]);
const numberish = z.union([z.number(), z.string().trim().transform(value => Number(value))]).refine(value => Number.isFinite(value), "Invalid number");
const intish = numberish.transform(value => Math.floor(value));
...
export const conversationCreateSchema = z.object({
  type: z.enum(["DIRECT", "GROUP", "ROOM"]).optional().default("DIRECT"),
...
export const messageSchema = z.object({
  ...
  metadata: jsonRecord.optional()
}).refine(value => value.receiverId || value.conversationId, "Receiver or conversation required").refine(value => Boolean(value.content || value.mediaUrl), "Message content or media required");
...
export const createRoomSchema = z.object({
  ...
  coverUrl: nullableUrlSchema.optional(),
  visibility: z.enum(["PUBLIC", "PRIVATE", "FOLLOWERS", "INVITE_ONLY"]).optional().default("PUBLIC"),
...
export const updateRoomSchema = z.object({
  ...
  topic: z.string().trim().max(80).optional(),
  coverUrl: nullableUrlSchema.optional(),
  visibility: z.enum(["PUBLIC", "PRIVATE", "FOLLOWERS", "INVITE_ONLY"]).optional(),
...
export const productCreateSchema = z.object({
  ...
  images: z.array(urlSchema).min(1).max(12),
  videoUrl: urlSchema.optional(),
  category: z.string().trim().min(1).max(120),
  subCategory: z.string().trim().max(120).optional(),
  brand: z.string().trim().max(120).optional(),
  stock: intish.min(0).max(1000000).optional().default(0),
  sku: z.string().trim().max(120).optional(),
  status: z.enum(["DRAFT", "ACTIVE", "INACTIVE", "SOLD_OUT", "ARCHIVED"]).optional().default("DRAFT"),
  metadata: jsonRecord.optional()
});
...
export const notificationCreateSchema = z.object({
  userId: objectIdLike,
  type: z.enum(["LIKE", "COMMENT", "FOLLOW", "MENTION", "MESSAGE", "ORDER", "SYSTEM", "ROOM", "GIFT", "TRADE", "MODERATION"]),
...
export const walletTransactionSchema = z.object({
  ...
  type: z.enum(["CREDIT", "DEBIT", "GIFT", "REFUND", "ADMIN_ADJUSTMENT", "REWARD"]),
  source: z.enum(["ADMIN", "GIFT", "TRADE", "ORDER", "REWARD", "SYSTEM"]).optional(),
  note: z.string().trim().max(500).optional(),
  metadata: jsonRecord.optional()
});
